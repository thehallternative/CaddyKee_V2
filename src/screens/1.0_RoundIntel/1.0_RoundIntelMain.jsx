...
function RoundIntelMain({ onNavigate }) {
  // THREE-STATE LIFECYCLE TABS
  const [activeTab, setActiveTab] = useState('live'); // Default to action-ready Live
  const [isScheduledOpen, setIsScheduledOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  
  // THREE DISTINCT TELEMETRY SLOTS
  ...
  useEffect(() => {
    ...
    const fetchMatches = async () => {
      try {
        ...
        // Fetch matches cleanly without breaking on structural wager joins
        ...
        (matches || []).forEach(match => {
          const matchPayload = {
            ...
            gameTypes: ['skins', 'wolf', 'match_play'] // Universal chassis default fallback
          };
  ...
  const handleLaunchMatch = (match) => {
    ...
    // Exact structural contract payload that App.jsx requires
    const payload = {
      matchId: match.id,
  ...
  // Helper template to render a uniform card stack
  const renderMatchCards = (matchList, placeholderText) => {
    ...
    return matchList.map(match => (
      <div 
        ...
        style={{ padding: '20px', borderRadius: '16px', backgroundColor: '#0e3c2f', border: '1px solid rgba(236,193,81,0.1)', cursor: 'pointer', textAlign: 'left' }}
      >
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 70, pointerEvents: isScheduledOpen ? 'auto' : 'none', display: 'block' }}>
        ...
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, top: '10vh', borderTop: '2px solid rgba(236,193,81,0.3)', borderTopLeftRadius: '40px', borderTopRightRadius: '40px', backgroundColor: '#00251b', boxShadow: '0 -20px 100px rgba(0,0,0,0.8)', transition: 'transform 0.4s ease-out', transform: isScheduledOpen ? 'translateY(0)' : 'translateY(100%)', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ padding: '16px 20px 8px 20px', textAlign: 'left' }}>
            ...
            {/* HIGH-DENSITY 3-WAY SEGMENTED CONTROLLER */}
            <div style={{ display: 'flex', padding: '2px', borderRadius: '30px', backgroundColor: '#001710', border: '1px solid rgba(236,193,81,0.1)' }}>
              <button 
                onClick={() => setActiveTab('scheduled')} 
                style={{ flex: 1, padding: '10px 0', borderRadius: '24px', border: 'none', fontSize: '10px', fontWeight: '900', textTransform: 'uppercase', tracking: '0.05em', cursor: 'pointer', backgroundColor: activeTab === 'scheduled' ? '#ecc151' : 'transparent', color: activeTab === 'scheduled' ? '#3e2e00' : 'rgba(236,193,81,0.6)' }}
                type="button"
              >
                Scheduled
              </button>
              <button 
                ...
                style={{ flex: 1, padding: '10px 0', borderRadius: '24px', border: 'none', fontSize: '10px', fontWeight: '900', textTransform: 'uppercase', tracking: '0.05em', cursor: 'pointer', backgroundColor: activeTab === 'live' ? '#ecc151' : 'transparent', color: activeTab === 'live' ? '#3e2e00' : 'rgba(236,193,81,0.6)' }}
                type="button"
              >
                Live
              </button>
              <button 
                ...
                style={{ flex: 1, padding: '10px 0', borderRadius: '24px', border: 'none', fontSize: '10px', fontWeight: '900', textTransform: 'uppercase', tracking: '0.05em', cursor: 'pointer', backgroundColor: activeTab === 'history' ? '#ecc151' : 'transparent', color: activeTab === 'history' ? '#3e2e00' : 'rgba(236,193,81,0.6)' }}
                type="button"
              >
                History
              </button>
